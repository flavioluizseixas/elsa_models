...
filename_rs <- paste0(path, "0_", ext, "_results.RData")
if (file.exists(filename_rs)) {
  load(filename_rs)
} else {
  for (ref in 4:7) {
    load(paste0(path, ref, "_", filename, "_", ext, "_results.RData"))
...
for (i in methods) {
  ...
  for (j in metricsbyClass) {
    ...
    g <- ggplot(df_g, aes(x = class_name, y = value, color = model, group = model)) +
      ...
      ggtitle(paste(metricsbyClass[which(metricsbyClass==j)], methodstxt[which(methods==i)], "-", ext)) +
  ...
  for (j in metricsOverall) {
    ...
    g <- ggplot(df_g, aes(x = class_name, y = value, color = model, group = model)) +
      geom_line() +
      scale_y_continuous(limits = c(0, 1)) +
      ggtitle(paste(metricsOverall[which(metricsOverall==j)], methodstxt[which(methods==i)], "-", ext)) +
  ...
  ggsave(paste0(path, i, "_", ext, ".png"), my_plot, width = 12, height = 5, dpi=150)
}
